...
class Sys573TestApp : public ArcadeTestApp
{
private:
    static constexpr AppInfo appInfo = makeAppInfo(
        "System 573 Test Mode", // name
        "NaokisRC",             // maker
        AppVersion(0, 0, 1)     // version
    );

public:
    static ArcadeTestApp *createArcadeApp(IAppHost *host) { return new Sys573TestApp(host); }
    static Application *createApp(IAppHost *host) { return new Sys573TestApp(host); }
    static constexpr const AppInfo &infoStatic() { return appInfo; }

    Sys573TestApp(IAppHost *host);

    static inline ArcadeTestApp *create(IAppHost *host)
    {
        return new Sys573TestApp(host);
    }
